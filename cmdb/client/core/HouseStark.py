...
class ArgvHandler(object):
    """
    客户端参数处理类模块，客户端模块执行参数：
    collect_data: 收集客户端参数
    run_forever: 后台运行，并定期执行服务器资源信息收集
    get_asset_id: 从服务端获取当前服务器的资产id
    report_asset:
    """
    ...
    def __update_asset_id(self, new_asset_id):
        """
        更新本地的资产id文件,第一次提交数据时没有资产id,先创建资产ID并返回ID，再提交硬件信息数据
        :param new_asset_id: 从服务端返回的资产ID编号
        :return:
        """
        asset_id_file = settings.Params['asset_id']
    ...
    def report_asset(self):
        ...
        # 如果服务器asset id存在就提交到asset_report对应url中
